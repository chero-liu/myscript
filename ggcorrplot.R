# from https://github.com/kassambara/ggcorrplot
...
ggcorrplot <- function(corr,
  ...
  corr <- as.matrix(corr)
  if (hc.order) {
    hc <- .hc_cormat_order(corr, dist.method = "euclidean", hc.method = hc.method)
    ord <- hc$order
    corr <- corr[ord, ord]
    if (!is.null(p.mat)) {
      p.mat <- p.mat[ord, ord]
  ...
  # adding colors
  p = p + ggplot2::scale_fill_gradientn(colors = colors, name = legend.title)
  # p <- p + ggplot2::scale_fill_gradient2(
  #   low = colors[1],
  #   high = colors[3],
  #   mid = colors[2],
  #   midpoint = quantile(corr$value)[3] ,
  #   #limit = c(-1, 1),
  #   space = "Lab",
  #   name = legend.title
  # )
  ...
  # 获取 X 轴标签文本
  labels <- unique(p$data[,rlang::as_name(p$mapping$x)])
  # 计算文本宽度（单位：pt）
  label_widths <- sapply(labels, function(label) {
    grob_width <- grid::grobWidth(grid::textGrob(label, gp = grid::gpar(fontsize = tl.cex)))
    grid::convertWidth(grob_width, "pt", valueOnly = TRUE)
  })
  # 获取最大宽度
  max_width <- max(label_widths)
  if (type == "lower") {
    p <- p + scale_y_discrete(position="right")+ ggplot2::theme(axis.text.x = ggplot2::element_text(vjust = 1, hjust = 1), plot.margin = margin( r = 5, l= max_width, unit = "pt"))
  } else if (type == "upper") {
    p <- p + scale_x_discrete(position="top") + ggplot2::theme(axis.text.x = ggplot2::element_text(vjust = 0, hjust = 0), plot.margin = margin( r= max_width, l= 5, unit = "pt"))
  }else{
    p <- p + ggplot2::theme(axis.text.x = ggplot2::element_text(vjust = 1, hjust = 1))
  }
  ...
  # if(hc.order & show_tree & type == "full"){
  #   phr <- hc %>% 
  #       ggtree(branch.length="none") + scale_x_reverse() + theme(plot.margin = margin(t=0, r=0, b=0 , l= 0))
  #   phc <- hc %>% 
  #       ggtree( branch.length="none") + layout_dendrogram() + theme_dendrogram(plot.margin = margin(t=0, r=0, b=0 , l= 0))
  #   print(class(phr))
  #   print(class(phc))
  #   p <- p %>% aplot::insert_right(phr, width = grid::unit(0.1, "npc")) %>% 
  #             aplot::insert_top(phc , height = grid::unit(0.1, "npc"))   
  # }
  return(p)
}
...
.hc_cormat_order <- function(cormat, dist.method = "euclidean", hc.method = "complete") {
  dd <- stats::as.dist((1 - cormat) / 2)
  dd <- stats::dist(dd, method = dist.method)
